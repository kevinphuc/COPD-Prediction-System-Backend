...
class HealthService:
    ...
    def _preprocess_audio_segment(self, audio_segment: np.ndarray, sr: int | float) -> np.ndarray:
        melspec = librosa.feature.melspectrogram(y=audio_segment, sr=sr, n_mels=128, fmax=8000)
    ...
    def _generate_spectrogram_image(self, audio_file_bytes: bytes) -> Tuple[bytes, Dict[str, Any], int]:
        try:
    ...
    def _run_prediction_model(self, audio_bytes: bytes) -> float:
        ...
        try:
            ...
            if "Both" in predictions: return 0.95
            elif "Wheezes" in predictions: return 0.85
            elif "Crackles" in predictions: return 0.50
            return 0.10
    ...
    def upload_new_health_data(self, session: Session, user_id: UUID, audio_bytes: bytes, audio_filename: str | None):
        db_input = None
        png_bytes = None
        ...
        try:
            ...
            # Upload (Có retry)
            try:
                self.storage.from_("spectrogram").upload(path=storage_path, file=png_bytes, file_options={"content-type": "image/png", "upsert": "true"})
            except:
                self.storage.from_("spectrogram").upload(path=storage_path, file=png_bytes, file_options={"content-type": "image/png", "upsert": "true"})
